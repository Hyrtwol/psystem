...
const
   ...
   maxaddr    = maxint;
   maxsp      = 49{39}; { number of standard procedures/functions }
   maxins     = 74;     { maximum number of instructions }
   maxids     = 250;    { maximum characters in id string (basically, a full line) }
   maxstd     = 48{39}; { number of standard identifiers }
   ...
   intdeff    = -1{11}; { default field length for integer }
   reldeff    = -1{22}; { default field length for real }
   chrdeff    = -1{1};  { default field length for char (usually 1) }
   boldeff    = -1{5};  { default field length for boolean (usually 5 for 'false') }
...
type
     ...
     { changed to operat because operator is a reserved keyword in fpc [tlc]}
     operat = (mul,rdiv,andop,idiv,imod,plus,minus,orop,ltop,leop,geop,gtop,
               neop,eqop,inop,noop);
...
var
    ...
    op: operat;                     (*classification of last symbol*)
    ...
    debug: boolean;                 { -- Debug checks }
                                    (*renamed list to lst because its a
                                      reserved keyword in irie [tlc]*)
    ...
    pipeptr: stp;                   (*pointer for pipe experiment[tlc]*)
    ...
    rop: array [1..maxres(*nr. of res. words*)] of operat;
    sop: array [char] of operat;
    ...
    errtbl: array [errornr] of boolean; { error occrence tracking }
    ...
    i: errornr; { index for error number tracking array }
...
(*-------------------------------------------------------------------------*)
  ...
  var i, j, f: integer; c: char; {m: boolean; not used [tlc]}
  begin
    i := 1; j := 1; f := 0;
...
(*-------------------------------------------------------------------------*)
  ...
  procedure writeextfile(var f: text; fp: extfilep);
  var c: char;
      i: 1..maxids;
  begin
    write(f,'''');
  ...
  begin
    ...
    writeln('Display:');
    writeln;
    for i := 0 to displimit do if display[i].fname <> nil then
    begin
       writeln('level ', i:1);
       writeln;
  ...
  begin case ferrnr of
    ...
    502: write('String type expected');
    end
  end;
  ...
  procedure insymbol;
    ...
    var i,k: integer;
        ...
        strval: csstr; {renamed string to strval because its a reserved keyword in fpc and delphi [tlc]}
    ...
    begin
      if eol then
      begin
        if lst then writeln(output);
        endofline
      end;
      if not eof(input) then
      begin
        eol := eoln(input); read(input,ch);
        if ord(ch) = 13 then read(input,ch); {fix for newlines on windows [tlc]}
        if lst then if ord(ch)>31 then write(output,ch);
        chcnt := chcnt + 1
      end
      else
      begin
        ...
        if ch <> '*' then
          begin
            if lcase(ch) = 't' then
              ...
              if lcase(ch) = 'l' then
                begin nextch; lst := ch = '+';
                  if not lst then writeln(output)
                end
              else
             if lcase(ch) = 'd' then
        ...
        begin lgth := 0; sy := stringconst;  op := noop;
          ...
          begin
            ...
            begin
              ...
              with lvp^ do
                begin slgth := lgth; strassvc(sval, strval, strglgth) end;
              val.intval := false;
              val.valp := lvp
            end
        end;
    ...
    if dodmplex then begin {  lexical dump }
      ...
      case sy of
         ...
         stringconst: begin write('string const: ''');
                            if val.intval then write(output, chr(val.ival))
...
1:  fcp := lcp
  ...
  procedure printtables(fb: boolean);
    ...
    begin
      if fp <> nil then
        with fp^ do
          if marked then
            begin marked := false; write(output,'S: ', stptoint(fp):intdig,' ', size:intdig, ' ');
              case form of
    ...
    begin
      if fp <> nil then
        with fp^ do
          begin write(output,'C: ', ctptoint(fp):intdig,' ');
  ...
  begin (*printtables*)
    ...
    writeln(output,'C: ', 'Entry #':intdig, ' ', 'Id':intdig, ' ', 'llink':intdig, ' ',
            'rlink':intdig, ' ', 'Typ':intdig, ' ', 'Class':intdig);
    writeln(output,'S: ', 'Entry #':intdig, ' ', 'Size':intdig, ' ', 'Form ':intdig);
    writeln(output,'===============================================================');
    marker;
    for i := top downto lim do
  ...
  procedure searchlabel(var llp: lbp; lvl: disprange);
  var fllp: lbp; { found label entry }
  begin
    fllp := nil; { set no label found }
    llp := display[lvl].flabel; { index top of label list }
  ...
  procedure block(fsys: setofsys; fsy: symbol; fprocp: ctp);
    ...
    function isstring(fsp: stp) : boolean; forward; {string is a reserved keyword in fpc and delphi [tlc]}
        ...
        if (fsp1 <> nil) and (fsp2 <> nil) then
          if fsp1^.form = fsp2^.form then
            case fsp1^.form of
              ...
              arrays: comptypes := isstring(fsp1) and isstring(fsp2) and
                                   (fsp1^.size = fsp2^.size );
    ...
    begin isstring := false;
      if fsp <> nil then
        if (fsp^.form = arrays) and fsp^.packing then begin
          ...
          if comptypes(fsp^.aeltype,charptr) and (fmin = 1) then isstring := true
        end
    end (*isstring*) ;
    ...
    (*^*)     if sy = arrow then
              begin
                new(lsp,pointer); pshstc(lsp); fsp := lsp;
                with lsp^ do
    ...
    procedure body(fsys: setofsys);
      ...
      begin topnew := topnew + cdx[i];
        if topnew > topmax then topmax := topnew
      end;

      procedure putic;
      begin
        if ic mod 10 = 0 then writeln(prr,'i ',ic)
      end;

      procedure gen0(fop: oprange);
      begin
        if prcode then begin putic; writeln(prr,mn[fop]) end;
        ic := ic + 1; mes(fop)
      end (*gen0*) ;
      ...
      begin
        if prcode then
          begin putic; write(prr,mn[fop]);
            if fop = 30 then
              begin
                writeln(prr,sna[fp2]);
                topnew := topnew + pdx[fp2];
                if topnew > topmax then topmax := topnew
              end
            else
              begin
                ...
                else if fop = 67 then writeln(prr,' ',fp2)
                else writeln(prr,' ',fp2);
                mes(fop)
              end
          end;
        ic := ic + 1
            ...
            case fop of
              ...
              47,48,49,52,53,55:
                begin write(prr,chr(fp1));
                  if chr(fp1) = 'm' then write(prr,' ',fp2);
                  writeln(prr)
                end;
              51: (*ldc*)
                case fp1 of
                  ...
                  3: writeln(prr,'b ',fp2);
                  4: writeln(prr,'n');
                  6: writeln(prr,'c ''',chr(fp2),'''');
                  5: begin
                       write(prr,'( ');
                       with cstptr[fp2]^ do
                         for k := setlow to sethigh do
                           (* increased for testing [sam] added space after [tlc] *)
      ...
      begin load;
        ...
        if prcode then begin putic; writeln(prr,mn[33],' l ',faddr);
        end;
        ic := ic + 1; mes(33)
      end (*genfjp*) ;
      ...
      begin
       if prcode then
          begin putic; writeln(prr, mn[fop], ' l ',fp2) end;
        ic := ic + 1; mes(fop)
      end (*genujpxjp*);
      ...
      begin
       if prcode then
          begin putic; writeln(prr, mn[fop],' ',fp1,' l ',fp2) end;
        ic := ic + 1; mes(fop)
      end (*genujpxjp*);
        ...
        if prcode then
          begin putic;
            ...
            begin { create ents or ente instructions }
              if fp1 = 1 then writeln(prr,mn[fop],'s',' l ',fp2)
              else writeln(prr,mn[fop],'e',' l ',fp2)
            end
            else
            writeln(prr,mn[fop],' ',fp1,' l ',fp2)
          end;
        ic := ic + 1; mes(fop)
      end;
      ...
      begin if prcode then writeln(prr, 'l ', labname)
      end (*putlabel*);
      ...
      procedure statement(fsys: setofsys);
        ...
        procedure call(fsys: setofsys; fcp: ctp);
          ...
          begin variable(fsys + [rparent]); loadaddress;
            ...
            if lkey <= 2 then begin
              if (gattr.typtr = textptr) or (gattr.typtr = pipeptr) then
          ...
          {var llev:levrange; not used [tlc]}
          begin
            {llev := 1; not used [tlc]}
            if sy = lparent then
            begin insymbol;
          ...
          procedure newdisposeprocedure;
            label 1;
            var lsp,lsp1: stp; {varts: integer; not used [tlc]}
                lsize: addrrange; lval: valu;
          begin variable(fsys + [comma,rparent]); loadaddress;
            lsp := nil; lsize := 0; {varts := 0; value assigned to 'varts' never used [tlc]}
            if gattr.typtr <> nil then
              with gattr.typtr^ do
                if form = pointer then
                  begin
            ...
            while sy = comma do
              begin insymbol;constant(fsys + [comma,rparent],lsp1,lval);
                {varts := varts + 1; value assigned to 'varts' never used [tlc]}
          ...
          begin
            ...
            begin
              ...
              {if gattr.typtr = nil then raise EP5.Create('gattr.typtr is not defined');}
              if (gattr.typtr <> textptr) and (gattr.typtr <> pipeptr) then error(116);
              ispipe := (gattr.typtr = pipeptr);
              if sy = comma then
                insymbol
              else
                error(20);
              ...
              with gattr do
                if typtr <> nil then
                begin
                  if (kind = cst) or (kind = varbl) then
                  begin
                    if typtr = charptr then
                    begin
                      gen2(51(*ldc*),1,typtr^.size);
                      load;
                    end
                    else if isstring(typtr) then
                    begin
            ...
            begin insymbol;
              ...
              begin
                if sy = rparent then
                  ...
                  error(4);
                { store the halt parameter in the exitcode var }
                gen1t(43(*sro*),40(*exitcodeoff*),gattr.typtr)
              end
            end;
            gen0(29(*stp*));
          end (*halt*);

          procedure peekprocedure;
          var lsp : stp;
          begin
            if sy = lparent then
            begin
              ...
              begin
                if sy = rparent then
                  ...
                  if sy <> comma then
                  begin error(116); skip(fsys + [rparent]) end
                  else
                  begin
                    insymbol; variable(fsys + [rparent]);
                    loadaddress;
                    gen2(51(*ldc*),1,gattr.typtr^.size);
                  end
              end
                ...
                begin
                  repeat
                    ...
                    case gattr.typtr^.form of
                      arrays, files:
                        gen2(51(*ldc*),1,ord(gattr.typtr^.form));
                      scalar,subrange,pointer,power,records,tagfld,variant:
                      begin
                        gen2(51(*ldc*),1,-1);
                        error(144) {Illegal type of expression}
                      end;
                    end;
                    params:=params+1;
                    test := sy <> comma;
          ...
          if fcp^.pfdeckind = standard then
            begin lkey := fcp^.key;
              if fcp^.klass = proc then
                begin
                  if not(lkey in [5,6,11,12,17,20,21,22,23,24,25,26]) then
                    if sy = lparent then insymbol else error(9);
                  case lkey of
                    1,2,3,4:       getputresetrewriteprocedure;
                    17:            pageprocedure;
                    5,11:          readprocedure;
                    6,12:          writeprocedure;
                    7:             packprocedure;
        ...
        procedure expression;
          var lattr: attr; lop: operat; typind: char; lsize: addrrange;

          procedure simpleexpression(fsys: setofsys);
            var lattr: attr; lop: operat; signed: boolean;

            procedure term(fsys: setofsys);
              var lattr: attr; lop: operat;
    ...
    begin (*body*)
      ...
      if fprocp <> nil then
        begin
          ...
          if prcode then
            begin
              writeln(prr,'l ',segsize,' = ',lcmax); {added a space in front of '=' to please delphi and fpc [tlc]}
              writeln(prr,'l ',stacktop,' = ',topmax) {added a space in front of '=' to please delphi and fpc [tlc]}
            end
        end
      else
              ...
              begin
                if not (strequri('input    ', filename) or
                  ...
                  begin
                    ...
                    if prcode then write(prr, ' = ', llcp^.vaddr);
                  end;
              end;
  ...
  begin
    ...
    na[39] := 'dispose  ';
    {new stuff tlc}
    na[40] := 'random   '; na[41] := 'randomize';
    na[42] := 'assign   '; na[43] := 'close    ';
    na[44] := 'exitcode '; na[45] := 'halt     ';
    na[46] := 'peek     '; na[47] := 'poke     ';
    na[48] := 'exec     ';
  end (*stdnames*) ;
  ...
  begin                                                 (*type underlying:*)
    ...
    new(pipeptr,files); pshstc(pipeptr);                (*pipe*)
    with pipeptr^ do
      begin filtype := charptr; size := filesize+charsize; form := files end
  end (*enterstdtypes*) ;
  ...
  begin                                                 (*name:*)
    ...
    new(cp,types); ininam(cp);                          (*pipe*)
    with cp^ do
      begin strassvr(name, 'pipe     '); idtype := pipeptr; klass := types end;
    enterid(cp);
    ...
    for i := 3 to 4 do
      begin
        new(cp,vars); ininam(cp);                       (*input,output*)
        with cp^ do
    ...
    for i:=33 to 34 do
      begin
        new(cp,vars); ininam(cp);                       (*prd,prr files*)
         with cp^ do
           begin strassvr(name, na[i]); idtype := textptr; klass := vars;
              vkind := actual; next := nil; vlev := 1;
    ...
    new(cp,func,standard); ininam(cp);                  (*random*)
    with cp^ do
      begin strassvr(name, na[40]); idtype := nil;
        pflist := nil; next := nil; key := 19;
        klass := func; pfdeckind := standard
      end; enterid(cp);
    new(cp,proc,standard); ininam(cp);                  (*randomize*)
    with cp^ do
      begin strassvr(name, na[41]); idtype := nil;
        pflist := nil; next := nil; key := 20;
        klass := proc; pfdeckind := standard
      end; enterid(cp);

    new(cp,proc,standard); ininam(cp);                  (*assign*)
    with cp^ do
      begin strassvr(name, na[42]); idtype := nil;
        pflist := nil; next := nil; key := 21;
        klass := proc; pfdeckind := standard
      end; enterid(cp);
    new(cp,proc,standard); ininam(cp);                  (*close*)
    with cp^ do
      begin strassvr(name, na[43]); idtype := nil;
        pflist := nil; next := nil; key := 22;
        klass := proc; pfdeckind := standard
      end; enterid(cp);

    new(cp,vars); ininam(cp);                           (*exitcode*)
    with cp^ do
      begin strassvr(name, na[44]);
        idtype := intptr; klass := vars;
        vkind := actual; next := nil; vlev := 1;
        vaddr := lcaftermarkstack+(4*(filesize+charsize));
      end; enterid(cp);
    new(cp,func,standard); ininam(cp);                  (*halt*)
    with cp^ do
      begin strassvr(name, na[45]); idtype := nil;
        pflist := nil; next := nil; key := 23;
        klass := proc; pfdeckind := standard
      end; enterid(cp);

    new(cp,proc,standard); ininam(cp);                  (*peek*)
    with cp^ do
      begin strassvr(name, na[46]); idtype := nil;
        pflist := nil; next := nil; key := 24;
        klass := proc; pfdeckind := standard
      end;
    enterid(cp);
    new(cp,proc,standard); ininam(cp);                  (*poke*)
    with cp^ do
      begin strassvr(name, na[47]); idtype := nil;
        pflist := nil; next := nil; key := 25;
        klass := proc; pfdeckind := standard
      end;
    enterid(cp);
    new(cp,proc,standard); ininam(cp);                  (*exec*)
    with cp^ do
      begin strassvr(name, na[48]); idtype := nil;
        pflist := nil; next := nil; key := 26;
        klass := proc; pfdeckind := standard
      end;
    enterid(cp);
  end (*entstdnames*) ;
  ...
  var i: integer;
  begin fwptr := nil;
    prtables := false; lst := true; prcode := true; debug := true;
    dp := true; errinx := 0;
    intlabel := 0; kk := maxids; fextfilep := nil;
    lc := lcaftermarkstack+filebuffer*(filesize+charsize)+intsize{exitcode[tlc]};
    ...
    for i := 1 to maxerr do errtbl[i] := false; { initialize error tracking }
  ...
  procedure inittables;
    ...
    begin
      ...
      { extra [tlc] }
      sna[40] :=' rni'; sna[41] :=' rnr'; sna[42] :=' rnz';
      sna[43] :=' asf'; sna[44] :=' asb'; sna[45] :=' asp';
      sna[46] :=' clo';
      sna[47] :=' pek'; sna[48] :=' pok';
      sna[49] :=' exe';
    end (*procmnemonics*) ;
    ...
    begin
      ...
      pdx[40] := -1; pdx[41] := -1; pdx[42] :=  0;
      pdx[43] := -1; pdx[44] := -1; pdx[45] := -1;
      pdx[46] := -1; pdx[47] := -1; pdx[48] := -1;
    end;
...
begin
  ...
  f := true;
  for i := 1 to maxerr do if errtbl[i] then begin
    if f then begin
      writeln;
